...
public class CreateGff4Vep {

    RgdGff3Dao dao = new RgdGff3Dao();
    Logger log = LogManager.getLogger("vep");
    ...
    public void createGff3(CreateInfo info) throws Exception{
        ...
        String fileName = info.getToDir() + "/" + ucscId + ".gff3";

        Gff3ColumnWriter gff3 = new Gff3ColumnWriter(fileName, info.getCompressMode());
        gff3.print(headerInfo);

        SequenceRegionWatcher sequenceRegionWatcher = new SequenceRegionWatcher(info.getMapKey(), gff3, dao);
        ...
        for( Gene gene: activeGenes ){
            ...
            String name = getName(gene, counters);
    ...
    /*
    column 9:
        ID - req for genes and transcriptss
        Parent - must have Parent
        biotype - req for transcripts (SO biotype) : "1 Ensembl transcript  1100  4900  . + . ID=transcript1;Name=GENE1-001;Parent=gene1;biotype=protein_coding"
     */
/*
...
*/
    ...
    private String getName(Gene gene, CounterPool counters) {
        ...
        if( name==null ){
            counters.increment(" Genes with NULL Name");
        }
        return name;
    }
